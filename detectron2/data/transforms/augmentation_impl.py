...
from .transform import ExtentTransform, ResizeTransform, RotationTransform, ResizePaddingTransform
...
class RandomResizePadding(Augmentation):
    """
    Scale the shorter edge to the given size, with a limit of `max_size` on the longer edge.
    If `max_size` is reached, then downscale so that the longer edge does not exceed max_size.
    """
    ...
    def __init__(
            self, shapes, interp=Image.BILINEAR):
        """
        Args:
            short_edge_length (list[int]): If ``sample_style=="range"``,
                a [min, max] interval from which to sample the shortest edge length.
    ...
    def get_transform(self, img):
        size = np.random.choice(np.arange(len(self.shapes)))
        size = self.shapes[size]
        return ResizePaddingTransform(img.shape[0], img.shape[1], size[0], size[1], self.interp)
